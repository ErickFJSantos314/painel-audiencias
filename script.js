...
function adicionarCardRealizado(audiencia) {
    ...
    card.style.gap = '10px';
    card.style.borderLeft = '6px solid #4F3128';
    card.style.padding = '10px';
    card.style.marginBottom = '8px';
    card.style.background = '#f7f5f2';
    card.style.width = '100%';
    card.style.boxSizing = 'border-box';

    card.innerHTML = `
        <span><strong>Data:</strong> ${audiencia.Data}</span> |
        <span><strong>Hora:</strong> ${audiencia.Hora}</span> |
        <span><strong>Processo:</strong> ${audiencia.Processo}</span> |
        <span><strong>Cliente principal:</strong> ${audiencia.Cliente}</span> |
        <span><strong>Advogado(a):</strong> ${audiencia.Advogado}</span> |
        <span><strong>Preposto(a):</strong> ${audiencia.Preposto}</span>
    `;
...
function preencherInfoMaisProxima() {
    if (!dadosHoje.length || indiceAtual >= dadosHoje.length) {
        document.getElementById('audiencia-destaque').textContent = 'Sem audiências';
    ...
    const audiencia = dadosHoje[indiceAtual];
    document.getElementById('audiencia-destaque').textContent = audiencia.Audiência || '';
